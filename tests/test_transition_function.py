...
def test_empty_initialization():
    ...
    assert isinstance(T, TransitionFunction), "Initialization failed: object is not an instance of TransitionFunction."
    assert len(T.T_dict) == 0, "Initialization failed: T_dict should be empty."
    assert T.T_dict[0][0][0] == 0.0, "Initialization failed: default value for T_dict should be 0.0."
    assert not T.sanity_check(), "Sanity check failed: probabilities do not sum to 1 for some (s,a) pairs."
...
def test_initialization_from_numpy_array():
    ...
    assert T.sanity_check(), "Sanity check failed: probabilities do not sum to 1 for some (s,a) pairs."
...
def test__get_item__():
    ...
    for s in range(num_s):
        for a in range(num_a):
            for s_next in range(num_s):
                assert T[s][a][s_next] == transition_array[s, a, s_next], f"__getitem__ failed: expected probability {transition_array[s, a, s_next]} for (s={s}, a={a}, s'={s_next}), got {T.T_dict[s][a][s_next]}."
