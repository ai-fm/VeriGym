...
def test_single_sample_dataset():
    ...
    assert R[state, action] == reward, f"Expected reward of 0 for single entry but found {R[0, 0]}"
    ...
    assert R_combined[state, action] == (reward+reward_2)/2, f"Expected reward of {reward} for combined dataset but found {R_combined[0, 0]}"
...
def test_multiple_samples_stochastic():
    ...
    for s in range(n_states):
        for a in range(n_actions):
            assert np.isclose(R[s,a], mean_reward, atol=0.1*mean_reward), f"Expected reward between {rewards.min()} and {rewards.max()} but found {R[s, a]}"